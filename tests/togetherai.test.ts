...
if (!TOGETHER_API_KEY) {
  console.warn("⚠️  process.env.TOGETHER_API_KEY is necessary to run these tests. Skipping...");
} else {
  ...
  describe("Conventional Commits", () => {
    test("Should generate conventional commit format", async () => {
    ...
    test("Should generate conventional commit for new feature", async () => {
      const gitDiff = await getDiff("new-feature.diff");
    ...
    test("Should generate conventional commit for refactoring", async () => {
