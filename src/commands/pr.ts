...
function parseRemoteUrl(remoteUrl: string): RepoInfo {
  ...
  const azureMatch = remoteUrl.match(
    /dev\.azure\.com[/:]([^/]+)\/([^/.]+)|vs-internal\.visualstudio\.com[/:]([^/]+)\/([^/.]+)/,
  );
  if (azureMatch) {
    return {
      provider: "azure",
      owner: azureMatch[1] || azureMatch[3],
      repo: azureMatch[2] || azureMatch[4],
    };
  }
...
export default command(
  ...
  () => {
    (async () => {
      if (!isInteractive()) {
        throw new KnownError("Interactive terminal required for PR creation.");
      }
      ...
      if (currentBranch.trim() === defaultBranch) {
        throw new KnownError(
          "PR creation requires being on a feature branch, not the default branch. Please switch to a feature branch with changes.",
        );
      }
      ...
      let diff;
      try {
        const { stdout } = await execa("git", ["diff", `origin/${defaultBranch}..HEAD`]);
        diff = stdout;
      } catch {
      ...
      // Count changed files
      const numFiles = diff.split("\n").filter((line) => line.startsWith("diff --git")).length;

      // Limit diff size to avoid token limits
      if (diff.length > MAX_DIFF_LENGTH) {
        diff = diff.substring(0, MAX_DIFF_LENGTH) + DIFF_TRUNCATED_SUFFIX;
      }
      ...
      generating.stop(`Generated PR content for ${numFiles} files in ${duration}s`);

      console.log(`${green("Title:")} ${title.replace(/\n/g, " ")}`);
      console.log(`${green("Body:")} ${body.length > 100 ? body.substring(0, 100) + "..." : body}`);

      const { text } = await import("@clack/prompts");
      const proceed = await text({
        message: "Press Enter to push and open PR creation in browser, or Ctrl+C to cancel",
      ...
      try {
        ...
        outro(green("PR creation page opened! Please review and submit the PR."));
      } catch {
